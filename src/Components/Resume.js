  ...
  render() {
    ...
    if(this.props.data){
      ...
      var projectImage = 'images/tech/'+skills.image;
        return <div key={skills.name} className="columns feature-item">
                  <img className='skill' alt={skills.name} src={projectImage} style={{height:"100px"}}/>
                  <h5>{skills.name}</h5>
    ...
    return (
      ...
      <div className="row skill">
         ...
         <div>
           <div className="nine columns main-col" style={{marginBottom:"50px"}}>
              <p className="lead center">{skillmessage}</p>
           </div>
				   <ul className="bgrid-quarters s-bgrid-thirds cf" style={{textAlign:"center"}}>
					  {skills}
					 </ul>
			  </div>
      </div>
